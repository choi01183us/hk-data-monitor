...
if (args.length > 1 || args.length === 1 && args[0] !== "--weather-only") throw new Error("用法：refresh-city.mjs [--weather-only]");
const results = await refreshCitySnapshots(args.length ? { kinds: ["weather"] } : {});
...
if (process.env.GITHUB_STEP_SUMMARY) {
  ...
  await appendFile(process.env.GITHUB_STEP_SUMMARY, [
    ...
    "", "新聞係政府公報；航班係前一日原定日期紀錄；天氣保留報告及標記各自時間。站內並非即時資訊。", "",
  ].join("\n"));
}
