...
import {label as displayLabel} from "./display-text.js";
...
export function publicExpenditureViews(indicator) {
  ...
  const label = (period) => `${period} ${displayLabel(indicator.period_notes?.[period] ?? "")}`.trim();
  const views = [];
  ...
  for (let i = periods.length - 1; i > 0; i--) {
    ...
    views.push({
      label: t(`${label(to)} 相比 ${label(from)}：增減金額`, `${label(to)} compared with ${label(from)}: change`),
      comparison: { from, to },
      indicator: {
        ...indicator,
        chart: { ...indicator.chart, period: to, label_zh: `名義增減(港元)：${to} 減 ${from}`, label_en: `Nominal change (HK$): ${to} minus ${from}` },
  ...
  for (const period of [...complete].reverse()) {
    views.push({
      label: t(`${label(period)}：開支金額`, `${label(period)}: expenditure`),
      indicator: { ...indicator, chart: { ...indicator.chart, period, label_zh: `港元(${label(period)})`, label_en: `HK$ (${label(period)})` } },
    });
  }
  return views;
}
