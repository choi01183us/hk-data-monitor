...
export function districtExplorer({population, income, mapUrl}) {
  ...
  const root = html`<section class="city-panel district-explorer" aria-labelledby="district-heading"><div class="city-panel-heading"><div><span class="monitor-kicker">01 / 社區與旅遊</span><h2 id="district-heading">同一座城市，不同生活。</h2></div><span class="monitor-tag">十八區</span></div><fieldset class="district-layers"><legend>揀一個地圖圖層</legend>${radios}</fieldset>${toolbar}${status}${mapView.element}${body}</section>`;
